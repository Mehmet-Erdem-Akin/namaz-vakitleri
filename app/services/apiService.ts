import { NextPrayer, PrayerTime } from "../types";
...
export const fetchAllPrayerTimes = async (city: string): Promise<PrayerTime> => {
    try {
        ...
        const response = await fetch(
            `https://api.collectapi.com/pray/all?data.city=${normalizedCity}`,
            {
                method: 'GET',
                headers: {
        ...
        const prayerTimes: PrayerTime = {
            fajr: data.result.find((item: Record<string, string>) => item.vakit === "İmsak")?.saat || "",
            sunrise: data.result.find((item: Record<string, string>) => item.vakit === "Güneş")?.saat || "",
            dhuhr: data.result.find((item: Record<string, string>) => item.vakit === "Öğle")?.saat || "",
            asr: data.result.find((item: Record<string, string>) => item.vakit === "İkindi")?.saat || "",
            maghrib: data.result.find((item: Record<string, string>) => item.vakit === "Akşam")?.saat || "",
            isha: data.result.find((item: Record<string, string>) => item.vakit === "Yatsı")?.saat || "",
        };
...
export const fetchNextPrayer = async (city: string, prayerName: string): Promise<NextPrayer> => {
    try {
        const normalizedCity = normalizeCity(city);
        const response = await fetch(
            `https://api.collectapi.com/pray/single?ezan=${prayerName}&data.city=${normalizedCity}`,
            {
                method: 'GET',
                headers: {
                    Authorization: `apikey ${API_KEY}`,
