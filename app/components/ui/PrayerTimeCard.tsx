    ...
    return (
        <div className={`rounded-lg shadow-md p-4 mb-4 ${isToday ? "bg-white border-l-4 border-blue-500" : "bg-gray-50"}`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                <TimeBox label="İmsak" time={data.prayerTimes.fajr} isNext={isToday && prayerInfo?.nextPrayer.name === "İmsak"} isCurrent={isToday && prayerInfo?.currentPrayer.name === "İmsak"} />
                <TimeBox label="Güneş" time={data.prayerTimes.sunrise} isNext={isToday && prayerInfo?.nextPrayer.name === "Güneş"} isCurrent={isToday && prayerInfo?.currentPrayer.name === "Güneş"} />
                <TimeBox label="Öğle" time={data.prayerTimes.dhuhr} isNext={isToday && prayerInfo?.nextPrayer.name === "Öğle"} isCurrent={isToday && prayerInfo?.currentPrayer.name === "Öğle"} />
                <TimeBox label="İkindi" time={data.prayerTimes.asr} isNext={isToday && prayerInfo?.nextPrayer.name === "İkindi"} isCurrent={isToday && prayerInfo?.currentPrayer.name === "İkindi"} />
                <TimeBox label="Akşam" time={data.prayerTimes.maghrib} isNext={isToday && prayerInfo?.nextPrayer.name === "Akşam"} isCurrent={isToday && prayerInfo?.currentPrayer.name === "Akşam"} />
                <TimeBox label="Yatsı" time={data.prayerTimes.isha} isNext={isToday && prayerInfo?.nextPrayer.name === "Yatsı"} isCurrent={isToday && prayerInfo?.currentPrayer.name === "Yatsı"} />
            </div>
        </div>
    );
};
...
const TimeBox = ({ label, time, isNext, isCurrent }: TimeBoxProps) => {
    ...
    return (
        <div className={className}>
            ...
            <div className={`font-semibold ${isNext ? "text-blue-700" : isCurrent ? "text-green-700" : "text-gray-800"}`}>
                {time}
            </div>
            {isCurrent && (
                <div className="text-xs mt-1 text-green-600 font-medium">Mevcut</div>
            )}
        </div>
    );
};
